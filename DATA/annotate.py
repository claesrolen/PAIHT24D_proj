...
class Annotate:
    def __init__(self, fname, out_path, LEN=3000, NFFT=256, NHOP=256, sr_out=16000):
        ...
        self.sr_out = sr_out
        sr_in0 = librosa.get_samplerate(self.fname)
        self.y, sr_in = librosa.load(self.fname, sr=sr_in0, mono=True)
            ...
            if len(self.t_vec) > 0:
                ...
                for ix in self.t_vec:
                    ...
                    wavfile.write(
                        ...
                        self.sr_out,
                        (32767*self.y[ix : (ix + self.LEN)]).astype(np.int16),
                    )
                    n += 1
                    strn += "," + str(ix)
                f.write(strn + "\n")
...
def main():
    ...
    fname = "./envdata/indoor/0_applause.wav"
    ann = Annotate(fname, out_dir,LEN=4096)
    ann.process()
